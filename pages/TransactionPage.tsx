  ...
  return (
    <div className="bg-transparent dark:bg-slate-950 min-h-screen pb-20 font-['Hind_Siliguri'] transition-colors duration-300">
      <div className="px-6 pt-8 pb-4 flex items-center gap-4 sticky top-0 glass-nav z-10">
        ...
        <h1 className="text-lg font-black text-slate-800 dark:text-slate-100">দান করুন</h1>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="glass-card p-6 rounded-[2.5rem]">
             <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400 rounded-2xl"><DollarSign className="w-6 h-6" /></div>
                <h3 className="font-black text-slate-800 dark:text-slate-200 text-sm uppercase tracking-widest">টাকার পরিমাণ</h3>
             </div>
             <input type="number" disabled={isSubmitting} className="w-full p-6 text-4xl font-black text-center bg-white/40 dark:bg-slate-900/40 backdrop-blur-md border-2 border-white/50 dark:border-slate-800/50 rounded-3xl outline-none focus:border-teal-500 transition-all text-teal-600 dark:text-teal-400 mb-6" placeholder="৳ 0" value={amount} onChange={e => setAmount(e.target.value)} />
             <div className="grid grid-cols-3 gap-3">
                {quickAmounts.map(amt => (
                  <button 
                    ...
                    className="p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-[10px] font-black text-slate-600 dark:text-slate-400 active:scale-95 transition-all hover:bg-white dark:hover:bg-slate-700 hover:border-teal-200 shadow-sm"
                  >
                    ৳{toBengaliNumber(amt)}
                  </button>
                ))}
          ...
          <div className="glass-card p-6 rounded-[2.5rem] space-y-6">
             <div className="flex items-center gap-3">
                <div className="p-3 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 rounded-2xl"><Coins className="w-6 h-6" /></div>
                <h3 className="font-black text-slate-800 dark:text-slate-200 text-sm uppercase tracking-widest">ফান্ড টাইপ</h3>
             </div>
             <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button 
                    ...
                    className={`p-4 rounded-2xl border-2 transition-all ${fundType === f.id ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-500' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}
                  >
                    <span className={`font-black text-[10px] ${fundType === f.id ? 'text-amber-700 dark:text-amber-300' : 'text-slate-400 dark:text-slate-500'}`}>{f.label}</span>
                  </button>
                ))}
             </div>
          </div>
          ...
          <div className="glass-card p-6 rounded-[2.5rem] space-y-6">
             <div className="flex items-center gap-3">
                <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-2xl"><Wallet className="w-6 h-6" /></div>
                <h3 className="font-black text-slate-800 dark:text-slate-200 text-sm uppercase tracking-widest">পেমেন্ট মেথড</h3>
             </div>
             <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button 
                    ...
                    className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${method === m.id ? 'bg-teal-50 dark:bg-teal-900/20 border-teal-500' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}
                  >
                    <div className={`w-10 h-10 ${m.color} rounded-xl flex items-center justify-center shadow-sm`}>
                       {m.icon}
                    </div>
                    <span className={`font-black text-[10px] ${method === m.id ? 'text-teal-700 dark:text-teal-300' : 'text-slate-400 dark:text-slate-500'}`}>{m.label}</span>
                  </button>
                ))}
             </div>
             {method === 'Bank' && (
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 space-y-3">
             <div className="flex items-center gap-3 mb-1">
                <div className="p-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-xl"><Hash className="w-5 h-5" /></div>
                <h3 className="font-black text-slate-800 dark:text-slate-200 text-[11px] uppercase tracking-widest">ভেরিফিকেশন</h3>
             </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-8">
           <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[3rem] p-8 text-center shadow-2xl animate-in zoom-in-95">
              <CheckCircle2 className="w-16 h-16 text-emerald-500 mx-auto mb-4" />
              <h3 className="text-2xl font-black text-slate-800 dark:text-slate-100 mb-2">সফল হয়েছে!</h3>
              ...
              <button onClick={() => navigate('/dashboard')} className="w-full py-4 bg-slate-900 dark:bg-slate-800 text-white rounded-2xl font-black text-xs uppercase shadow-xl">ঠিক আছে</button>
           </div>
        </div>
      )}
    </div>
