  ...
  return (
    <div className="bg-gradient-to-br from-[#f8fafc] via-[#e2e8f0] to-[#cbd5e1] dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 min-h-screen pb-24 font-sans transition-colors duration-300">
      <div className="px-5 pt-6 pb-2 flex items-center justify-between sticky top-0 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl z-30 border-b border-white/20 dark:border-slate-800/50">
        ...
        <div className="flex gap-2">
          <button 
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-slate-900 dark:bg-slate-800 text-white rounded-xl text-[9px] font-black shadow-lg uppercase tracking-wider border border-white/10"
          >
          ...
          <button 
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-teal-600 dark:bg-teal-700 text-white rounded-xl text-[9px] font-black shadow-lg shadow-teal-200/50 dark:shadow-teal-900/40 uppercase tracking-wider disabled:opacity-50"
          >
      ...
      <div className="px-3 mt-4" ref={reportRef}>
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-[2.5rem] shadow-2xl shadow-slate-900/10 overflow-hidden border border-white dark:border-slate-800 max-w-4xl mx-auto transition-colors duration-300">
          ...
          <div className="bg-teal-50/50 dark:bg-slate-800/50 grid grid-cols-2 border-b border-teal-100/50 dark:border-slate-800">
            <div className="p-5 border-r border-teal-100/50 dark:border-slate-800">
              <p className="text-[9px] font-black text-teal-600 dark:text-teal-400 uppercase tracking-widest mb-1">Contributor</p>
              <p className="text-sm font-black text-black dark:text-white uppercase truncate">{currentUser?.name}</p>
            </div>
            
            <div className="p-5 flex justify-between items-center">
              <div>
                <p className="text-[9px] font-black text-teal-600 dark:text-teal-400 uppercase tracking-widest mb-1">Report Year</p>
                <p className="text-sm font-black text-black dark:text-white">{selectedYear}</p>
              </div>
              <div className="flex gap-2">
                <button 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[11px] border-collapse">
              <thead>
                <tr className="bg-slate-900 dark:bg-slate-950 text-white font-black uppercase tracking-wider">
              ...
              <tbody>
                {transactions.length > 0 ? (
                  transactions.map((tx, index) => (
                    ...
                    >
                      <td className="py-4 px-4 border-r border-white/50 dark:border-slate-800 font-black text-black dark:text-slate-200 text-center">{index + 1}</td>
                      <td className="py-4 px-3 border-r border-white/50 dark:border-slate-800 text-center font-black text-black dark:text-slate-200">
                        {tx.date.split('-').reverse().slice(0, 2).join(' / ')}
                      </td>
                      <td className="py-4 px-3 border-r border-white/50 dark:border-slate-800 text-center font-black text-black dark:text-slate-200 uppercase">{getDayName(tx.date)}</td>
                      <td className="py-4 px-3 border-r border-white/50 dark:border-slate-800 text-center font-black text-black dark:text-slate-200">
                      ...
                      <td className="py-4 px-3 border-r border-white/50 dark:border-slate-800 text-center font-black text-black dark:text-slate-200 bg-teal-200/40 dark:bg-teal-900/40">
                        ৳{tx.amount}
                      </td>
                      <td className="py-4 px-4 font-black text-black dark:text-slate-200 text-[9px] uppercase tracking-tighter">
                  ...
                  <tr>
                    <td colSpan={7} className="py-24 text-center">
                      <div className="flex flex-col items-center opacity-30">
                        ...
                        <p className="text-[12px] font-black uppercase tracking-widest text-black dark:text-white">No Records Found</p>
                      </div>
                    </td>
                  </tr>
                )}
              ...
              <tfoot>
                <tr className="bg-teal-900 dark:bg-teal-950 text-white">
                  <td className="py-6 px-6 text-right font-black uppercase text-[10px] tracking-[0.2em] border-r border-white/10 dark:border-slate-800" colSpan={5}>
                    Yearly Total Contribution
                  </td>
                  <td className="py-6 px-3 text-center font-black text-teal-300 dark:text-teal-400 text-sm shadow-2xl">
        ...
        <div className="mt-8 text-center pb-8">
           <p className="text-[9px] font-black text-slate-600 dark:text-slate-500 uppercase tracking-[0.4em]">Together for Humanity • Unity Care Foundation</p>
        </div>
      </div>
